...
export class RecipeDetailsComponent implements OnInit, OnDestroy {
      ...
      (params: Params) => {
        ...
        this.store.select('recipes').pipe(take(1)).subscribe(state => {
          state.recipes.find(recipe => {
            if (recipe._id == this.id) {
              this.recipe = recipe;
              this.image = "http://localhost:8080/" + recipe.image;
            }
          });
        })
      }
  ...
  onEdit() {
    this.router.navigate(['/recipes/all/recipe/', this.recipe._id,'edit']);
  }

  onDelete() {
    this.store.dispatch(new RecipesActions.DeleteRecipe(this.recipe._id));
  }
